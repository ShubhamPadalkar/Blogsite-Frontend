...
import { updatePasswordAction } from "../../redux/reducers/userSlice";
...
const UpdatePassword = () => {
  ...
  const {passwordUpdated , loading } = useSelector(state => state?.users)

  if(passwordUpdated){
    setTimeout(() => {
      navigate(`/profile/${passwordUpdated?._id}`)
    }, 2000);
  }
